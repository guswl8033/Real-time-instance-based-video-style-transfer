...
class StaticCenterCrop(object):
    ...
    def __call__(self, img):
        new_img = img[(self.h - self.th) // 2:(self.h + self.th) // 2, (self.w - self.tw) // 2:(self.w + self.tw) // 2, :]

        # new_width, new_height = len(new_img[0]), len(new_img)
        # data = sum(new_img, [])  # ugly hack to flatten the image
        # im = Image.new('RGB', (new_width, new_height))
        # im.putdata(data)
        # im.save('/mnt/ITRC/flownet2-pytorch/testC/new_img/test.png')
        return new_img
...
class ImagesFromFolder(data.Dataset):
  def __init__(self, args, is_cropped, root = '/mnt/ITRC/flownet2-pytorch/dataset/LetItGo_test', iext = 'jpg', replicates = 1):
    self.args = args
    self.is_cropped = is_cropped
    self.crop_size = args.crop_size
    self.render_size = args.inference_size
...
class ARtists(data.Dataset):
    def __init__(self, args, is_cropped = False, root = '', dstype = 'clean', replicates = 1):
        ...
        for file in file_list:
            ...
            img1 = join(image_root, fprefix + "%03d" % (fnum + 0) + '.jpg')
            img2 = join(image_root, fprefix + "%03d" % (fnum + 1) + '.jpg')
            if not isfile(img1) or not isfile(img2) or not isfile(file):
                continue

            self.image_list += [[img1, img2]]
            self.flow_list += ['/mnt/ITRC/MPISintelDataset/flownet/flow/alley_1/frame_0001.flo']
            # self.flow_list += [file]
        self.size = len(self.image_list)
...
class ARtistsClean(ARtists):
    def __init__(self, args, is_cropped = False, root = '', replicates = 1):
        super(ARtistsClean, self).__init__(args, is_cropped = is_cropped, root = root, dstype = 'clean', replicates = replicates)


class ImagesFromFolder(data.Dataset):
    def __init__(self, args, is_cropped, root='/mnt/ITRC/flownet2-pytorch/dataset/test', iext='jpg', replicates=1):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
        self.render_size = args.inference_size
